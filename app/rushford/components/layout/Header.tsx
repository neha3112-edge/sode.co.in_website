...
export function Header() {
  ...
  return (
    <header className="fixed left-0 top-0 z-50 w-full bg-white">
      {/* Screenshot wali top navy line */}
      <div className="h-[6px] w-full bg-[#101b30]" />
            ...
            >
              {navItems.map((item) => (
                <a
                  key={item.label}
                  href={item.href}
                  className="relative py-5 text-[16px] font-semibold tracking-[0.01em] text-[#172033] transition-colors duration-200 hover:text-[#056493]"
                >
                  {item.label}
                </a>
              ))}
      ...
      {/* Mobile Navigation */}
      <div
        className={`overflow-hidden border-gray-200 bg-white shadow-md transition-all duration-300 lg:hidden ${
          isMenuOpen
